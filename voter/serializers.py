...
class NCVHisSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = NCVHis
        fields = (
            'ncid', 'county_id', 'voter_reg_num', 'election_desc',
            'voting_method', 'voted_party_cd', 'pct_description',
            'voted_county_id', 'vtd_description',
        )


class ChangeTrackerSerializer(serializers.ModelSerializer):
...
class NCVoterSerializer(serializers.ModelSerializer):
    ...
    voting_history = NCVHisSerializer(many=True, read_only=True)
    change_history = ChangeTrackerSerializer(many=True, read_only=True)

    class Meta:
        model = NCVoter
        fields = (
            'voting_history', 'change_history',
            'ncid', 'county_id', 'birth_age', 'confidential_ind',
            'birth_state', 'age', 'county_desc', 'voter_reg_num', 'status_cd',
            'voter_status_desc', 'reason_cd', 'voter_status_reason_desc',
            'absent_ind', 'name_prefx_cd', 'last_name', 'middle_name',
            'first_name', 'midl_name', 'name_sufx_cd', 'res_street_address',
            'res_city_desc', 'state_cd', 'zip_code', 'mail_addr1',
            ...
            'municipality_abbrv', 'municipality_desc', 'ward_abbrv',
            'ward_desc', 'cong_dist_abbrv', 'super_court_abbrv',
